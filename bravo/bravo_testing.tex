...
\section{Functional Testing Report}
	...
	As can be seen by the following sections unfortunately this team failed to implement the required functionality. As a result of this the evaluation of the classes provided was taken up and an attempt to provide some report on the provided code.
    ...
    \subsection{Implemented Functionality short-comings (with reference to pre- and post-conditions)}
		...
		ResearchCategoryAssociation was not implemented as specified in the domain model, with no reference to ResearcherCategory and ResearcherCategoryState. Instead, it appears as though they made use of an enumeration of hard coded categories such as STUDENT, MEMBER and GROUPLEADER in stead of an association to ResearchCategory. Unless the team was instructed to implement it in this fashion, they have not adhered to the functional requirements. Additionally, ResearchCategoryAssociation makes reference to a ResearchGroupType, which in the domain model has nothing to do with Research Groups. Additionally person does not have a ResearchGroupAssociation.
		...
		Instead of aggregation, the team made use of composition which, again, is not what was specified. Once again it appears they have both over simplified and over complicated elements of researchGroups instead of just adhering to the domain model. In this case a Group does not even inherit from an Entity.

		Finally Organizations are not an association of a Person but more of a aggregation again not as specified in the domain model.
